...
type UseFormHookArgumentType<T extends FieldValues> = UseFormProps<T> & { schema: NopeObject };
type ReturnType<T extends FieldValues> = UseFormReturn<T, object> & { errors: FormState<T>['errors'] };

export function useForm<T extends FieldValues>({ ...options }: UseFormHookArgumentType<T>): ReturnType<T> {
